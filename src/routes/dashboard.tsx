...
export const Dashboard = () => {
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      await deleteDoc(doc(db, "userAnswers", id));

       // Also update your local state if you’re storing interviews in state
      ...
      <div className="md:grid md:grid-cols-3 gap-3 py-4">
        {loading ? (
          ...
          interviews.map((interview) => (
            <InterviewPin key={interview.id} interview={interview} onDelete={handleDelete} />
          ))
        ) : (
